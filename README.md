...
└── outputs/                             # Auto-created on first run; all figures and CSVs (gitignored)
    └── thesis_figures/                  # Section 9: the ten publication figures (300 dpi)
```
...
slot) is no longer used in the docx, but Section 9 still regenerates it under its old name
on every run since Section 9 is also append-only.
